#include <commands/ImportPairedAnimation.h>
...
static bool BeginConversion(const string& importSkeleton, const string& importSkeleton2, const string& importFBX, const string& cacheFilePath, const string& behaviorFolder, const string& exportPath);
static void InitializeHavok();
static void CloseHavok();


ImportPairedAnimation::ImportPairedAnimation()
{
}
...
string ImportPairedAnimation::GetName() const
{
	return "ImportPairedAnimation";
}

string ImportPairedAnimation::GetHelp() const
{
	string name = GetName();
	transform(name.begin(), name.end(), name.begin(), ::tolower);

	// Usage: ck-cmd ImportPairedAnimation
	string usage = "Usage: " + ExeCommandList::GetExeName() + " " + name + " <path_to_skeleton_hkx> <path_to_skeleton_hkx_2> <path_to_fbx_animation> [--b=<path_to_behavior_folder>] [--c=<path_to_cache_file>] [--e=<path_to_export>]\r\n";

	const char help[] =
		R"(Converts a paired FBX animation to HKX. Requires two preexisting HKX skeleton
		
		Arguments:
			<path_to_skeleton_hkx> the animation skeleton in hkx format
			<path_to_skeleton_hkx_2> the animation skeleton in hkx format
...
string ImportPairedAnimation::GetHelpShort() const
{
	return "TODO: Short help message for ImportFBX";
}

bool ImportPairedAnimation::InternalRunCommand(map<string, docopt::value> parsedArgs)
{
	//We can improve this later, but for now this i'd say this is a good setup.
	string importFBX, importSkeleton, importSkeleton2, exportPath, cacheFilePath,
		behaviorFolder;;

	importSkeleton = parsedArgs["<path_to_skeleton_hkx>"].asString();
	importSkeleton2 = parsedArgs["<path_to_skeleton_hkx_2>"].asString();
	...
	InitializeHavok();
	BeginConversion(importSkeleton, importSkeleton2, importFBX, cacheFilePath, behaviorFolder, exportPath);
	CloseHavok();
	return true;
}

bool BeginConversion(const string& importSkeleton, const string& importSkeleton2, const string& importFBX, const string& cacheFilePath, const string& behaviorFolder, const string& exportPath) {
	bool batch = false;
	...
	if (!fs::exists(importSkeleton2) || !fs::is_regular_file(importSkeleton2)) {
		Log::Error("Invalid file: %s", importSkeleton2.c_str());
		return false;
	}
	...
	if (!batch)
	{
		...
		wrangler.setExternalPairedSkeletonPath(importSkeleton2);
