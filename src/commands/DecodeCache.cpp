...
bool DecodeCacheCmd::InternalRunCommand(map<string, docopt::value> parsedArgs)
{
	...
	fs::path cache_root_folder = fs::path(source_havok_project_cache) / "refresh";

	cache.save(cache_root_folder / "animationdatasinglefile.txt", cache_root_folder / "animationsetdatasinglefile.txt");
