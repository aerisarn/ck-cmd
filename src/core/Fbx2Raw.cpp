...
static void ReadMesh(RawModel &raw, FbxScene *pScene, FbxNode *pNode, const std::map<const FbxTexture *, FbxString> &textureLocations)
{
	...
	Mat4f scaleMatrix = Vec4f(scaleFactor, scaleFactor, scaleFactor, 1.0f).asDiagonal();//Mat4f::FromScaleVector(Vec3f(scaleFactor, scaleFactor, scaleFactor));
...
{    
    ...
    for (size_t animIx = 0; animIx < animationCount; animIx++) {
        ...
        if (verboseOutput) {
            printf("animation %zu: %s (%d%%)", animIx, (const char *) animStackName, 0);
        }
...
{
    if (fs::exists(fbxFileName)) {
        return fbxFileName;
    }
    // Get the file name with file extension.
    const std::string fileName = fs::path(fbxFileName).filename().string();
    ...
    const std::string fileBase = fs::path(fbxFileName).filename().replace_extension("").string();

    // Try to find a match without file extension.
    for (const auto &file : directoryFileList) {
        // If the two extension-less base names match.
        if (StringUtils::CompareNoCase(fileBase, fs::path(fbxFileName).filename().replace_extension("").string()) == 0) {
    ...
    FbxScene *pScene, const char *fbxFileName, const char *extensions, std::map<const FbxTexture *, FbxString> &textureLocations)
{
    ...
    const std::string folder = fs::path(fbxFileName).parent_path().string();

    // Check if there is a filename.fbm folder to which embedded textures were extracted.
    const std::string fbmFolderName = folder + fs::path(fbxFileName).filename().string() + ".fbm/";

    // Search either in the folder with embedded textures or in the same folder as the FBX file.
    const std::string searchFolder = fs::exists(fbmFolderName) ? fbmFolderName : folder;
	...
	for (auto & p : fs::directory_iterator(searchFolder))
		if (MatchExtension(p.path().extension().string().c_str(), extensions))
			fileList.emplace_back(p.path().filename().string());
