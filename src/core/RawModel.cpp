    ...
    std::vector<RawModel> &materialModels, bool shortIndices, const int keepAttribs, const bool forceDiscrete) const
{
    ...
    if (invertedTransparencySort) {
        ...
        for (const auto &triangle : triangles) {
            ...
            if (textureIndex < 0) {
                if (vertices[triangle.verts[0]].color[3] < 1.0f ||
                    vertices[triangle.verts[1]].color[3] < 1.0f ||
                    vertices[triangle.verts[2]].color[3] < 1.0f) {
...
{
    const float l0 = (vertices[verts[1]].position - vertices[verts[0]].position ).squaredNorm();
    const float l1 = (vertices[verts[2]].position - vertices[verts[1]].position ).squaredNorm();
    const float l2 = (vertices[verts[0]].position - vertices[verts[2]].position ).squaredNorm();
    ...
    if (e0.squaredNorm() < FLT_MIN || e1.squaredNorm() < FLT_MIN) {
        return VEC3F_ZERO;
    }
    auto result = e0.cross(e1);
    auto resultLengthSquared = result.squaredNorm();
...
size_t RawModel::CalculateNormals(bool onlyBroken)
{
    ...
    for (int vertIx = 0; vertIx < vertices.size(); vertIx ++) {
        ...
        if (onlyBroken && (vertex.normal.squaredNorm() >= FLT_MIN)) {
            continue;
        }
        vertex.normal = VEC3F_ZERO;
        if (onlyBroken) {
    ...
    for (int vertIx = 0; vertIx < vertices.size(); vertIx ++) {
        ...
        RawVertex &vertex = vertices[vertIx];
        if (vertex.normal.squaredNorm() < FLT_MIN) {
