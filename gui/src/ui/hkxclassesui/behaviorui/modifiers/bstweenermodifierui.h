#ifndef BSTWEENERMODIFIERUI_H
#define BSTWEENERMODIFIERUI_H

#include <QGroupBox>

#include "src/utility.h"


class QGridLayout;
class TableWidget;
class LineEdit;
class QuadVariableWidget;
class CheckBox;
class GenericTableWidget;
class DoubleSpinBox;

namespace UI {

class HkxObject;
class BSTweenerModifier;
class hkbVariableBindingSet;

class BSTweenerModifierUI final: public QGroupBox
{
    Q_OBJECT
public:
    BSTweenerModifierUI();
    BSTweenerModifierUI& operator=(const BSTweenerModifierUI&) = delete;
    BSTweenerModifierUI(const BSTweenerModifierUI &) = delete;
    ~BSTweenerModifierUI() = default;
public:
    void loadData(HkxObject *data);
    void connectToTables(GenericTableWidget *variables, GenericTableWidget *properties);
    void variableRenamed(const QString & name, int index);
signals:
    void viewVariables(int index, const QString & typeallowed, const QStringList &typesdisallowed);
    void viewProperties(int index, const QString & typeallowed, const QStringList &typesdisallowed);
    void modifierNameChanged(const QString & newName, int index);
private slots:
    void setName(const QString &newname);
    void setEnable();
    void setTweenPosition();
    void setTweenRotation();
    void setUseTweenDuration();
    void setTweenDuration();
    void setTargetPosition();
    void setTargetRotation();
    void viewSelected(int row, int column);
    void setBindingVariable(int index, const QString & name);
private:
    void toggleSignals(bool toggleconnections);
    void selectTableToView(bool viewproperties, const QString & path);
private:
    static const QStringList headerLabels;
    BSTweenerModifier *bsData;
    QGridLayout *topLyt;
    TableWidget *table;
    LineEdit *name;
    CheckBox *enable;
    CheckBox *tweenPosition;
    CheckBox *tweenRotation;
    CheckBox *useTweenDuration;
    DoubleSpinBox *tweenDuration;
    QuadVariableWidget *targetPosition;
    QuadVariableWidget *targetRotation;
};
}
#endif // BSTWEENERMODIFIERUI_H
