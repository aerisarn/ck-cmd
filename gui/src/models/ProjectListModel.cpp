...
QVariant ProjectListModel::data(const QModelIndex& index, int role) const
{
    ...
    if (index.row() >= _cache.getNumProjects())
        return QVariant();

    if (role == Qt::DisplayRole)
        return QString::fromStdString(_cache.project_at(index.row()));
    else
        return QVariant();
}
...
bool ProjectListModel::insertRows(int position, int rows, const QModelIndex& parent)
{
...
bool ProjectListModel::removeRows(int position, int rows, const QModelIndex& parent)
{
